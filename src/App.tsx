...
className={`aspect-square flex flex-col items-center justify-center rounded-xl border-none transition-all select-none ${isFree ? 'bg-[#00a0e3] cursor-default' : active ? 'bg-[#002b5e] scale-95 shadow-inner cursor-pointer' : 'bg-slate-100 hover:bg-slate-200 cursor-pointer active:scale-90'}`}>
...
if (activeTab === ‘depart’)  return […(s.pre || []), …(s.rules || [])];
if (activeTab === ‘onboard’) return […(s.app  || []), …(s.onboard || [])];
return s[activeTab] || [];
})();
